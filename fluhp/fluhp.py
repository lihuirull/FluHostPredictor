...
def load_adaptability_markers(filepath):
    """
    Load and process the adaptability markers from an Excel file.

    Parameters:
        filepath: Path to the Excel file containing adaptability markers.

    Returns:
        Dictionary with protein types as keys and lists of human residues as values.
    """
    adapt_data = pd.read_excel(filepath, engine = "openpyxl")
    adapt_data['Human residues'] = adapt_data['Human residues'].str.split("(").str.get(0)
    return adapt_data.groupby('Protein Type')['Human residues'].apply(list).to_dict()


def load_receptor_markers(filepath):
    """
    Load and process the receptor markers from an Excel file.

    Parameters:
        filepath: Path to the Excel file containing receptor markers.

    Returns:
        Dictionary with original protein types as keys and lists of markers as values.
    """
    recep_data = pd.read_excel(filepath, engine = "openpyxl")
    recep_data['Mutation'] = recep_data['Mutation'].apply(lambda x: re.search(r"\d+[A-Z]", x).group())
    return recep_data.groupby('Original Protein Type')['Mutation'].apply(list).to_dict()


def map_residues_to_h3(protein, marker_dict, convert_to_h3_dict):
    """
    Maps the residue numbers for a given protein to the H3 numbering system.
...
def annotate_markers(adaptability_path, receptor_path, use_receptor = True):
    ...
    adapt_dict = load_adaptability_markers(adaptability_path)
    recep_dict = {}
    if use_receptor:
        recep_dict = load_receptor_markers(receptor_path)
...
def renumber_proteins(fasta_path, acc_pro_dict, marker_dict):
    ...
    for record in fasta_sequences:
        ...
        is_ha_type = protein_abbr in HA_TYPES
        ...
        if protein_abbr in marker_dict or is_ha_type:
            try:
                ...
                if is_ha_type:
                    # Store the renumbered sequence for HA types
                    HA_results[protein_abbr] = renumber_sequence(best_alignment)
                else:
                    # Store the renumbered sequence for non-HA types
...
def identify_adaptive_markers(input_file_path, renumbering_results, marker_markers, acc_pro_dic, output_directory = ".",
                              prefix = ""):
    """
    Identifies adaptive markers in protein sequences based on the provided marker markers
    ...
    Returns:
        pandas.DataFrame: A dataframe with columns for protein IDs, adaptive markers, count of adaptive markers,
        and protein types.
    """
    results = []
    os.makedirs(output_directory, exist_ok = True)
    ...
    for acc_id, renumbered_position in renumbering_results.items():
        ...
        for marker in expected_markers:
            ...
            if match and match.group() in renumbered_position:
                adaptive_markers.append(match.group())

        # Calculate the number of adaptive markers found
        num_adaptive_markers = len(adaptive_markers)

        # Append the findings to the results list
        results.append({
            'Strain ID': input_file_name.split(".")[0],
            'Adaptive markers': ','.join(adaptive_markers),
            'Number of Adaptive markers': num_adaptive_markers,
...
def process_extract_cmd(input_file, args, is_directory = True):
    ...
    results_df = identify_adaptive_markers(
